...
(1)暂停比分选取时机为:**领先时被对手追分(momentum)、对方连续得分(momentum)、比分临近局点或赛点时情况(winning rate)最多;**双方比分相差不超过2分以及比分为0:3和9:8时为主要暂停时机;局比分为0:2、1:3和2:3时为主要的暂停局分选取时机,且在局中分布主要集中在中局阶段和尾局阶段。(win
