/*
 * udpate: Modularize momentum calculations
 */
...
// 蒙特卡洛模拟实时获胜概率（保持不变）
PDD winningRate(double cap1, double cap2, int scr1, int scr2) {
    int batch_size = 10000;
    int win1 = 0, win2 = 0;
    std::uniform_real_distribution<double> distribution(0.0, cap1 + cap2);
...
const double cap1 = 0.45;     // 实力值（A)
const double cap2 = 0.55;     // 实力值（B）
...
// 提取的势能计算函数：输入当前场景参数，返回L_i、G_A、G_B、M_A、M_B
std::tuple<double, double, double, double, double> calculate_momentum(
    ...
    rtwp_win = winningRate(cap1, cap2, scrA + 1, scrB).first;
    rtwp_lose = winningRate(cap1, cap2, scrA, scrB + 1).first;
    L_i = rtwp_win - rtwp_lose;

    // 2. 计算杠杆获取量G_A/G_B（文档定义）
    double ga = (winner == PLAYER_A) ? L_i : 0.0;
    double gb = (winner == PLAYER_B) ? -L_i : 0.0;

    all_points.push_back({ga, gb, game_idx});

    // 3. 计算势能M_A/M_B（5球窗口+跨局权重调整）
...
int main() {
    ...
    for (int game_idx = 0; game_idx < game_seqs.size(); ++game_idx) {
        ...
        // 遍历本局每一分
        for (char winner : seq) {
            // 调用函数计算势能相关值（当前分尚未加入all_points，故索引为all_points.size()）
            auto [L_i, ga, gb, M_A, M_B] = calculate_momentum(
                all_points,
                all_points.size(), // 当前分的索引（添加前的size）
                game_idx,
                winner,
                scrA,
                scrB,
